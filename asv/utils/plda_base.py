...
class PldaEstimation(object):
    ...
    def plda_write(self,plda):
        ...
        with open(plda,'w') as f:
            mean = list(map(str, list(self.mean.reshape(1,-1)[0])))
            within_var = list(map(str, list(self.within_var.reshape(1,-1)[0])))
            between_var = list(map(str, list(self.between_var.reshape(1,-1)[0])))
            
            f.write('mean {}\n'.format(' '.join(mean)))
            f.write('within_var {}\n'.format(' '.join(within_var)))
            f.write('between_var {}\n'.format(' '.join(between_var)))
    '''
